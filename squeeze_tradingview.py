"""
╔══════════════════════════════════════════════════════════════════════════════╗
║   SQUEEZE INDEX v2.0 — Modelo de Compresión por Física de Ondas             ║
║   Inspirado en Jim Simons / Renaissance Technologies                         ║
║   Mejoras: vectorización, episodios, backtest real, análisis espectral      ║
╚══════════════════════════════════════════════════════════════════════════════╝
"""
...
st.title("〰️ SqueezeIndex v2")
st.caption(f"Ejecución: {datetime.now(UTC).strftime('%Y-%m-%d %H:%M:%S UTC')} · Física de Ondas + Backtest Real")
...
def compute_lambda_vectorized(smoothed: pd.Series, window: int, use_spectrum: bool) -> pd.Series:
    """
    Calcula Lambda (longitud de onda dominante) de forma vectorizada.
    
    Dos métodos:
      1. Welch PSD  → frecuencia dominante por análisis espectral (más robusto)
      2. Peaks      → distancia media entre extremos (método clásico mejorado)
    
    Retorna Series de Lambda para cada barra.
    """
    n = len(smoothed)
    ...
    for i in range(window - 1, n):
        ...
        if use_spectrum and len(seg) >= 16:
            # Welch PSD — estima densidad espectral de potencia
            try:
                freqs, psd = welch(seg, nperseg=min(len(seg), 8))
                freqs = freqs[freqs > 0]
                psd   = psd[1:]  # descarta DC
                if len(psd) > 0:
...
def compute_trend(df: pd.DataFrame, smoothed: pd.Series, lam: pd.Series) -> pd.Series:
    """
    Trend compuesto (4 componentes, pesos calibrados):
    
    1. Slope_long  (0.35) — pendiente normalizada por ATR en ventana = Lambda
    2. Slope_short (0.25) — pendiente rápida (6 barras) para reactividad
    3. MFI_score   (0.25) — Money Flow Index centrado en [-1, 1]
    4. ROC_score   (0.15) — Rate of Change momentum relativo al ATR
    
    Todos los componentes → tanh para limitar outliers.
    """
    n = len(df)
    prices_arr  = smoothed.values
    ...
    # Slopes vectorizadas con polyfit por ventana dinámica
    for i in range(n):
    ...
    # ROC score (momentum de 5 barras normalizado por ATR)
...
def detect_squeeze_episodes(squeeze_on: pd.Series, min_duration: int = 3) -> pd.Series:
    """
    Etiqueta episodios consecutivos de SqueezeOn (grupos de días contiguos).
    Retorna Serie con ID de episodio (0 = sin squeeze).
    """
...
def calculate_squeeze_index(
    ...
    """
    Pipeline completo del modelo SqueezeIndex v2.
    
    Indicadores producidos:
      EMA, STD, BB (Upper/Lower/Width), ATR,
      KC (Upper/Lower), Lambda, SqueezeIndex,
      SqueezeOn, SqueezeEpisode, Trend,
      Direction, SqueezeDetected, SignalStrength
    """
    ...
    # ── Lambda (longitud de onda dominante) ───────────────────────────────
    smoothed   = df["Close"].ewm(span=5, adjust=False).mean()
    df["Lambda"] = compute_lambda_vectorized(smoothed, window, use_spectrum)

    # ── SqueezeIndex: energía acumulada ───────────────────────────────────
    # v2: añadimos factor de persistencia (BBWidth percentil relativo) y
    # penalizamos lambdas muy cortas (ruido) o muy largas (sin señal)
    bb_percentile = df["BBWidth"].rolling(window * 3).rank(pct=True).fillna(0.5)
    # 1 - percentile: cuanto MÁS estrecho el BB, mayor el índice (invertimos)
    compression_factor = (1 - bb_percentile).clip(0, 1)
    
    # Lambda óptima: ni muy corta (<3) ni muy larga (>window)
    lambda_quality = np.exp(-0.5 * ((df["Lambda"] - window / 3) / (window / 4)) ** 2)
    
    raw_si = compression_factor / df["BBWidth"].replace(0, np.nan)
    # Normalizamos el SqueezeIndex a escala [0, 100]
    ...
    # ── SqueezeDetected + fuerza de señal ────────────────────────────────
    df["SqueezeDetected"] = df["SqueezeOn"] & (df["Trend"].abs() > threshold)
    
    # SignalStrength: combinación de SqueezeIndex y |Trend|
    df["SignalStrength"] = (
...
# ══════════════════════════════════════════════════════════════════════════════
# BACKTEST REAL (EPISODIOS)
# ══════════════════════════════════════════════════════════════════════════════

def run_backtest(df: pd.DataFrame, forward_days: int = 5) -> pd.DataFrame:
    """
    Backtest riguroso por EPISODIO de squeeze:
    
    Lógica correcta:
      - Al FINALIZAR un episodio SqueezeOn (último día del grupo), registra:
          · Dirección prevista (Trend del último día del episodio)
          · SqueezeIndex máximo del episodio
          · Duración del episodio
      - Mide el retorno real en los N días SIGUIENTES al fin del episodio
      - Compara: ¿la dirección del retorno coincide con la prevista?
    
    Esto es lo matemáticamente correcto — el squeeze PRECEDE al movimiento.
    """
    records = []
...
def build_main_chart(df: pd.DataFrame, asset_name: str) -> go.Figure:
    ...
    fig = make_subplots(
        ...
        subplot_titles=(
            ...
            "SqueezeIndex v2 (energía acumulada, escala 0-100)",
            "Trend Compuesto (4 componentes)",
            "Lambda Ω — Longitud de Onda Dominante (días)",
        )
    )
    ...
    fig.update_layout(
        ...
        title=dict(text=f"〰️ {asset_name} — Ondas de Compresión", font=dict(size=16))
    )
...
def build_backtest_chart(bt: pd.DataFrame) -> go.Figure:
    """Visualiza resultados del backtest por episodio."""
    if bt.empty:
        return go.Figure()
    ...
    fig.update_layout(
        title="Retorno post-squeeze por episodio (barras verdes = acierto)",
        template="plotly_dark",
        height=380,
...
def send_email_resend(to_email: str, subject: str, body: str):
    ...
    try:
        r = requests.post(
            ...
            json={"from": "SqueezeIndex v2 <onboarding@resend.dev>",
...
with tab_dash:
    if update:
        ...
        if df_raw.empty:
            ...
            with st.spinner("Calculando modelo v2…"):
...
with tab_backtest:
    st.subheader("🎯 Backtest Real — Episodios de Compresión")
    st.markdown("""
    **Metodología correcta**: se registra la dirección prevista al *finalizar* cada episodio de squeeze
    y se mide el retorno real en los N días *siguientes*. Así medimos si el squeeze *predice* el breakout,
    no si la dirección es correcta durante la compresión (que no tiene sentido estadístico).
    """)
    ...
    if update:
        if "df" in dir() and not df.empty and "SqueezeEpisode" in df.columns:
            ...
            if bt.empty:
                st.warning("No hay episodios suficientes (mínimo 3 días consecutivos) en el rango seleccionado.")
            else:
                hits    = (bt["Acierto"] == "✅").sum()
                total   = len(bt)
                ...
                avg_si_miss  = bt.loc[bt["Acierto"] == "❌", "SI_Max"].mean()
    ...
    if scan_btn:
        ...
        for idx, (name, tk) in enumerate(asset_list):
            ...
            df_s = calculate_squeeze_index(
                df_s, window, bb_mult, kc_mult, atr_period, threshold, use_spectrum
            )
            if len(df_s) < 10:
                continue
            last_s  = df_s.iloc[-1]
        ...
        if results:
            ...
            if st.button("Enviar reporte", key="scan_send"):
                ...
                body = (
                    f"SqueezeIndex v2 — {datetime.now(UTC).strftime('%Y-%m-%d %H:%M UTC')}\n\n"
...
with tab_info:
    ...
    | Componente | v1 | v2 |
    |---|---|---|
    | **Lambda** | Peaks O(n²) | Welch PSD vectorizado |
    | **SqueezeIndex** | `1/BBWidth * 1/Lambda` simple | Percentil relativo + calidad Lambda |
    | **Trend** | 3 componentes | 4 componentes (slope_long + slope_short + MFI + ROC) |
    | **Episodios** | Días aislados | Grupos de ≥3 días consecutivos |
    | **Backtest** | Dirección del día en squeeze (incorrecto) | Retorno N días POST-squeeze (correcto) |
    | **Señal** | Binaria (on/off) | Gradiente (SignalStrength 0–1) |

    ---

    ### Lambda Ω — Longitud de onda dominante

    Se estima con el **método Welch** (estimador de densidad espectral de potencia):
    - Divide la ventana temporal en segmentos solapados
    - Calcula la FFT de cada segmento y promedia
    - Identifica la frecuencia con mayor potencia → `f_dom`
    - `Lambda = 1 / f_dom` (días por ciclo)

    Esto es más robusto que contar peaks/valleys porque:
    - Es resistente al ruido de alta frecuencia
    - Detecta la periodicidad **estadísticamente dominante**, no la más reciente
    - No depende de la escala del precio
    ...
    ```
    compression_factor = 1 - percentil_rolling(BBWidth, 3*window)
    lambda_quality     = gaussian(Lambda, μ=window/3, σ=window/4)
    SI_raw             = compression_factor / BBWidth
    SqueezeIndex       = normalize(SI_raw * lambda_quality, 0-100)
    ```

    - El **percentil relativo** evita que el índice sea artificialmente alto en activos
      con baja volatilidad estructural (ej: EURUSD vs BTCUSD no son comparables en escala)
    - `lambda_quality` penaliza lambdas muy cortas (ruido) o muy largas (sin periodicidad útil)

    ---

    ### Trend compuesto (4 factores)

    | Factor | Peso | Qué mide |
    |---|---|---|
    | Slope_long | 35% | Momentum estructural (ventana = Lambda) |
    | Slope_short | 25% | Momentum táctico (6 barras) |
    | MFI score | 25% | Presión compradora/vendedora (volumen-price) |
    | ROC score | 15% | Rate of change normalizado por ATR |

    Todos normalizados vía `tanh` para limitar outliers y mantener rango [-1, 1].

    ---

    ### Por qué el backtest v1 era incorrecto

    En v1, se comparaba la **dirección del Trend durante el squeeze** con la **variación diaria del mismo día**.
    Esto mide si el modelo es internamente consistente, no si predice el futuro.

    En v2, el backtest mide si la dirección prevista al **final del episodio** coincide con el
    retorno real en los **N días siguientes**. Eso es lo que importa para operar.
    """)

st.caption("SqueezeIndex v2 · Física de Ondas + Backtest Riguroso")
